  ...
  inputSchema: {
    ...
    properties: {
      indices: {
        ...
        description: "List of index symbols to fetch (e.g., ^GSPC for S&P 500, ^DJI for Dow Jones)",
...
function isMarketDataArgs(args: unknown): args is { indices?: string[] } {
  return (
    ...
    args !== null &&
    (!("indices" in args) || 
     Array.isArray((args as { indices: any }).indices))
  );
}
...
async function fetchStockQuote(symbol: string) {
  ...
  try {
    ...
    throw new Error(`Failed to fetch stock quote: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
function formatBasicQuote(chartData: any, symbol: string): string {
  try {
    ...
    return `
Symbol: ${symbol}
Name: ${meta.instrumentInfo?.shortName || meta.instrumentInfo?.longName || symbol}
...
Change: $${formatNumber(quote.regularMarketChange)} (${formatPercent(quote.regularMarketChangePercent)})
Previous Close: $${formatNumber(quote.regularMarketPreviousClose)}
Open: $${formatNumber(quote.regularMarketOpen)}
Day Range: $${formatNumber(quote.regularMarketDayLow)} - $${formatNumber(quote.regularMarketDayHigh)}
...
function formatMarketData(indices: any[]): string {
  return indices.map(index => {
    const changePercent = index.regularMarketChangePercent ? 
      (index.regularMarketChangePercent * 100).toFixed(2) + '%' : 'N/A';
    
    return `
${index.shortName || index.longName || index.symbol}
Price: ${index.regularMarketPrice?.toLocaleString() || 'N/A'}
Change: ${index.regularMarketChange?.toLocaleString() || 'N/A'} (${changePercent})
Previous Close: ${index.regularMarketPreviousClose?.toLocaleString() || 'N/A'}
Day Range: ${index.regularMarketDayLow?.toLocaleString() || 'N/A'} - ${index.regularMarketDayHigh?.toLocaleString() || 'N/A'}
`.trim();
  }).join('\n\n');
}

async function fetchStockHistory(symbol: string, period: string = "1mo", interval: string = "1d") {
  checkRateLimit();
  ...
  try {
    ...
    throw new Error(`Failed to fetch stock history: ${error instanceof Error ? error.message : String(error)}`);
  }
}

function formatStockHistory(historyData: any, symbol: string, period: string, interval: string): string {
  try {
    ...
    if (meta.firstTradeDate && meta.regularMarketTime) {
      result += `Trading Period: ${new Date(meta.firstTradeDate * 1000).toLocaleDateString()} to ${new Date(meta.regularMarketTime * 1000).toLocaleDateString()}\n\n`;
    } else {
      result += '\n';
    }
    ...
    for (let i = 0; i < timestamps.length; i += step) {
      ...
      result += `${date.padEnd(11)} | $${open.padEnd(8)} | $${high.padEnd(8)} | $${low.padEnd(8)} | $${close.padEnd(8)} | ${volume}\n`;
    }
    ...
    return `Error formatting stock history: ${error instanceof Error ? error.message : String(error)}`;
  }
}
